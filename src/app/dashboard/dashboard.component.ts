...
export class DashboardComponent implements OnInit {
  ...
  constructor(private firestore: AngularFirestore) {
      ...
      // var citiesRef = firestore.collectionGroup('stock', ref => ref.where('user', '==', 'thusharadahanayaka@gmail.com')).snapshotChanges()._subscribe((data){
      //
      // });
  ...
  getFirestoreData(date) {
      ...
      this.firestore.collection('outlets').snapshotChanges().subscribe((data) => {
          this.tableData =[];
          data.map(e => {
              var x = e.payload.doc.data();
              // console.log(e.payload.doc.data());
              this.outlets[x['name']] = x;
    ...
    getTime(timestamp: any) {
        return new Date(timestamp.seconds * 1000).toLocaleTimeString().split(" ")[0];;
    }
}
